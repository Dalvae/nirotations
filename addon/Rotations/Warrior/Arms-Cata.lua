local queue = {
	...
	"Slam",
	"Strike",
	"InnerRageBloodFury", -- Added Inner Rage and Blood Fury
	"DeadlyCalmBloodFury", -- Added Deadly Calm and Blood Fury
	"AutoAttack"
}
...
local abilities = {
	...
	["BattleShout"] = function()
		if ni.spell.available(spells.CommandingShout.id) and not ni.player.buff(spells.CommandingShout.id) then
			ni.spell.cast(spells.CommandingShout.name)
			return true
		end
	end,
	["AutoAttack"] = function()
	...
	-- end,
	["MortalStrike"] = function()
		if ValidUsable(spells.MortalStrike.id, t) and FacingLosCastBerserk(spells.MortalStrike.name, t) then
			return true
		end
	end,
	...
	["InnerRageBloodFury"] = function()
		if ni.spell.available(spells.InnerRage.id) and ni.spell.available(spells.BloodFury.id) then
			ni.spell.castspells(spells.InnerRage.name .. "|" .. spells.BloodFury.name)
			return true
		end
		return false
	end,
